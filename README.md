...
SET GOOS=windows
SET GOARCH=amd64
go build -ldflags "-H=windowsgui" -o sftpupload.exe
```

Build for macOS (tray mode):
```bash
# arm64 (Apple Silicon)
GOOS=darwin GOARCH=arm64 go build -o sftpupload-macos-arm64
# amd64 (Intel Macs)
GOOS=darwin GOARCH=amd64 go build -o sftpupload-macos-amd64
```
...
```bat
go build -o sftpupload.exe
sftpupload.exe
```

## GitHub Actions (auto build + release)
- Every push/PR runs `go test` and `go build` on Windows.
- Tagging a release (e.g., `v1.2.3`) creates a GitHub Release and attaches a Windows build (`watcher.exe`) for auto-update distribution. Push a new tag to publish an updated binary.
